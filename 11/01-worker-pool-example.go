// Example copied from https://gobyexample.com/worker-pools
package main
...
func main() {
	...
	//This starts up 4 workers, initially blocked because there are no jobs yet.
	for w := 1; w <= 4; w++ {
		go worker(w, jobs, results)
	}

	//Here we send 20 jobs and then close that channel to indicate that’s all the work we have.
	for j := 1; j <= numJobs; j++ {
		jobs <- j
	}
	close(jobs)
